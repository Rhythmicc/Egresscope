...
# 2026-08-13 · 代理花费追踪小流量过滤

## 视觉与交互验证

- 参考截图：手机端追踪列表中仍出现 `2.ntp.ubuntu.com · 11.0 KB` 与 `ms-vscode.gallery.vsassets.io · 10.7 KB`，小额连接淹没了真正需要关注的花费记录。
- 最终手机状态：`qa/traffic-ledger-significant-mobile.jpg`（390 × 844）；参考与成品并排图：`qa/traffic-ledger-significant-comparison.png`（780 × 844）。
- 默认选择 `≥ 1 MiB`，仅保留按“设备 + 目标 + 出口类型”聚合后达到阈值的组合；手机列表不再出现 KB 级记录。
- `全部流量` 是同级显式切换，点击后测试用的 `2.ntp.ubuntu.com · 11 KB` 会立即恢复，避免为视觉简化牺牲取证能力。
- 原始连接明细始终请求全部数据，因此大流量组合内的小额成员连接不会被阈值误删。
- 390 × 844 下筛选器保留在横向控制条中，卡片、操作按钮和底部导航均无重叠或页面级横向溢出。

## 数据语义验证

- 阈值在数据库聚合完成后通过 `HAVING` 等价条件执行，不是仅过滤当前前端分页。
- NAS 生产数据验证：59 个代理流量组合中默认展示 14 个，隐藏 45 个低于 1 MiB 的组合；默认结果最小值为 1,362,705 B。
- 后端 38 项测试通过，生产构建通过，健康检查同时确认 mihomo 与数据库正常。
...
- 判定语义统一为“同一设备访问同一目标，5 分钟滚动累计流量”；卡片和设置弹窗不再使用容易误解的“单连接阈值”。
- 5 GiB 以下的连接事件仍保留在花费追踪中，但不会进入 AI 分类或自动处置。
- 卡片结构、响应式断点和现有视觉令牌均未改变；生产构建通过，无新增布局溢出风险。
