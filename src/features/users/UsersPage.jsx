...
import { api } from "../../api";
import { GeneratedPasswordField } from "./PasswordDialog";
...
export function UsersPage({ demoMode = false }) {
  ...
  const openCreate = () => {
    setEditingId(null);
    setForm({ username: "", password: generateRandomPassword(), role: "viewer", devices: "" });
    setShowForm(true);
  };
      ...
      <section className="panel users-panel">
        ...
        {users.map(user => (
          <div className="user-row" key={user.id}>
            ...
            <span className="device-scope">{user.role === "admin" ? "全部设备" : user.allowedDevices.length ? user.allowedDevices.join("、") : "全部设备"}</span>
            <span className="boundary-copy"><span>{user.role === "admin" ? "全局读写" : user.allowedDevices.length ? "指定设备只读" : "全局只读"}</span><button type="button" onClick={() => openEdit(user)}>编辑</button></span>
          </div>
        ))}
      </section>
      {showForm && (
        <div className="modal-backdrop" onMouseDown={() => setShowForm(false)}>
          <form className="user-modal" onMouseDown={event => event.stopPropagation()} onSubmit={submit}>
            ...
            <GeneratedPasswordField label={editingId ? "新密码（留空不修改）" : "初始密码"} required={!editingId} value={form.password} onChange={password => setForm({ ...form, password })} />
            ...
            {form.role === "viewer" && <label>可见设备 IP（留空可查看全部）<input value={form.devices} onChange={event => setForm({ ...form, devices: event.target.value })} placeholder="留空为全部，或填写 192.168.31.28, 192.168.31.46" /></label>}
